...
exports.updateTeacher = (req, res, next) => {
    ...
    teacherDto.update({ _id: req.body.id }, teacher, (err, data) => {
        ...
        if (req.body.oldDocumment) {
            ...
            userDto.update({ username: req.body.oldDocumment }, user, (err, u) => {
                if (err) {
                    return res.status(400).json(
                        {
                            error: err
...
exports.getByDocument = (req, res, next) => {
    teacherDto.getByDocument({ findOneAndUpdate: req.params.document }, (err, data) => {
        if (err) {
            return res.status(400).json(
                {
                    error: err
